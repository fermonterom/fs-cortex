...
| `test_cx_eod_gather.sh` | 17 | **(v3.38.1)** `core/_cx-eod-gather.sh`: root/non-git detection, name-merge across subdir+root, cross-OS basename, 24h rolling window, registry hash→name, error counting, JSON shape, `_archive` skip, MultiEdit capture; `--write` mode (summary file, intraday trace accumulation, Quick Resume + For tomorrow present, default JSON writes nothing, newline-injection sanitized, no lock/tmp leftovers) |
