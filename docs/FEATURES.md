# fs-cortex v3.6 — Feature Reference
...
- **9 error detection patterns**: error, failed, exception, traceback, fatal, panic, segfault, OOM, command not found
...
- **4 pattern detectors**:
  ...
  3. User corrections (same file edited 2+ times)
...
1. **Jaccard dedup**: Unicode-safe tokenization (word boundaries + CJK characters), configurable threshold (default 0.80)
...
3. **Staleness scoring**: 0-100 based on age since last_seen (7d=0, 30d=30, 60d=60, 90d+=90+). Auto-archive at threshold (default 90). Confidence decay proportional to staleness
...
**Decay**: confidence decays proportionally to staleness score. 90-day stale instincts auto-archive.
...
- **Path traversal protection** on backup archive extraction
- **Token budget cap** (8000/session, reflexes exempt)
...
- Path traversal validation on backup archive import
...
## Tests (8 suites, 124 tests)

| Suite | Tests | Coverage |
|---|---|---|
| `test_security.sh` | 7 | Injection, command injection, scrubbing, validation |
| `test_dream_cycle.sh` | 26 | Jaccard, contradictions, staleness, regex, health |
...
| `test_hooks_e2e.sh` | 13 | Full pipeline: observe→inject→learn with mock data |

### CI
- GitHub Actions: macOS + Linux × Python 3.9/3.12 × Node 18/22
- ShellCheck + flake8 linting
